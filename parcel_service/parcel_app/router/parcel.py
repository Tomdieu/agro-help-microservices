...
async def list_parcels():
    parcels = await Parcel.all().select_related('cultures')
    print('Parcels',parcels)
    return parcels
    # return await ParcelListSchema.from_queryset(parcels)

@router.get('/{id}/',status_code=200,response_model=schema.Parcel,responses={404:{"model":HTTPNotFoundError}})
async def get_parcel(id:int):
    parcel = await Parcel.filter(id=id).first()
    if not parcel:
        raise HTTPException(status_code=404,detail="parcel with id={id} not found".format(id=id))
    return parcel

# @router.post('',response_model=schema.Parcel)
@router.post('/',response_model=schema.Parcel,status_code=201)
async def create_parcel(parcel:schema.ParcelIn_Pydantic,user:User=Depends(authenticate_token)):
...
# @router.patch('/{id}',response_model=schema.Parcel)
async def update_parcel(id:int,parcel:schema.ParcelIn_Pydantic,user:User=Depends(authenticate_token)):
...
@router.post("/{id}/add_cultures",response_model=dict,summary='This route add a list of cultures to a parcel')
async def add_cultures(id:int,cultureIds:schema.CultureIds):
    ...
    return {"message":f"{len(cultures_added)} cultures added to you parcel successfully"}


@router.post("/{id}/suggest_culture/",response_model=dict,summary="This route suggest a list of culture for a parcel",status_code=200)
